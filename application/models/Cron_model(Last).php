<?php
log_activity('CRON: run() method was entered');
use Carbon\Carbon;
   ...
   public function run($manually = false)
{
    if ($this->can_cron_run()) {
        error_log("CRON TEST: write test\n", 3, APPPATH . "logs/custom-log.txt");
        log_message('info', 'CRON: run() method was entered');
        ...
        if ($manually == true) {
            ...
            log_message('info', 'Cron Invoked Manually');
        }
    }
            ...
            log_activity('CRON: Running create_recurring_invoices()');
            $this->load->model('invoices_model');
            $this->invoices_model->create_recurring_invoices();
