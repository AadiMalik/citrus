...
if (empty($vat_number)) {
    // Backup: extract from formatted organization info
    $org_info = format_organization_info();
    if (preg_match('/VAT\s*Number:\s*([A-Z0-9]+)/i', $org_info, $match)) {
        $vat_number = $match[1];
    }
}

// 🧾 Dynamic heading (VAT Invoice or Invoice)
$invoice_heading = !empty($vat_number) ? 'VAT Invoice' : 'Invoice';

$info_right_column .= '<span style="font-weight:bold;font-size:27px;">' . $invoice_heading . '</span><br />';
